import{useEffect as Ne,useMemo as we,useState as N}from"react";import{useAppApi as Ke}from"@kirocrew/app-sdk";import{Card as I,
CardTitle as W,Btn as J,StatCard as G,ContentSkeleton as Ie,EmptyState as ve,PageHeader as We}from"@kirocrew/app-sdk/ui";import{jsx as y,jsxs as L}from"react/jsx-runtime";var S={top:8,right:6,bottom:20,left:42};function De(t){if(t<=0)return 1;
let e=10**Math.floor(Math.log10(t)),r=t/e;return(r<=1?1:r<=2?2:r<=5?5:10)*e}var Te=t=>t>=1e4?`${Math.round(t/1e3)}k`:t>=
1e3?`${(t/1e3).toFixed(1).replace(/\.0$/,"")}k`:String(Math.round(t*10)/10);function Le(t,e){return Math.max(1,Math.ceil(
t/e))}function ne({width:t,height:e,max:r,labels:s,labelOf:u,maxLabels:l=12,children:d}){let c={w:t-S.left-S.right,h:e-S.
top-S.bottom},i=De(r),g=h=>c.h-h/i*c.h,f=[0,.25,.5,.75,1].map(h=>i*h),b=Le(s.length,l);return y("svg",{className:"ul-svg",
...
axis",x:-6,y:g(h)+3,textAnchor:"end",children:Te(h)})]},h)),d(g,c),s.map((h,m)=>m%b===0||m===s.length-1?y("text",{className:"\
...
children:(f,b)=>t.map((h,m)=>{let z=0,C=(m+.5)*i-g/2;return L("g",{children:[y("title",{children:`${r?r(h):h} \xB7 ${Math.
round(d[m]*10)/10}${s}`}),e.map(P=>{let $=P.values[m]??0;if($<=0)return null;let _=f(z+$),o=f(z);return z+=$,y("rect",{className:"\
...
Ee=t=>`${t.slice(0,7)}-01T00`;function ze(t){let e=Number(t.slice(0,4)),r=Number(t.slice(5,7)),[s,u]=r===1?[e-1,12]:[e,r-
1];return`${String(s).padStart(4,"0")}-${String(u).padStart(2,"0")}-01T00`}function Pe(t){let e=t.length?t[t.length-1]:new Date().
toISOString().slice(0,13),r=Ee(e);return{source:"assumed-local-month",resets:"",start_utc:"",start_hour:r,prev_start_hour:ze(
r),end_hour:""}}var Oe=["hours","model","surface","agent","job","session"];function pe(t){let e=t&&typeof t=="object"?t:
{},r=e.dims&&typeof e.dims=="object"?e.dims:{},s={};for(let c of Oe)s[c]=Array.isArray(r[c])?r[c]:[];let u=Array.isArray(
...
tz=="string"?e.tz:"",window_days:Number(e.window_days)||0,shards:Number(e.shards)||0,cycle:d?Pe(s.hours):e.cycle,official:e.
official&&typeof e.official=="object"?e.official:{},dims:s,rows:u,labels:{session:e.labels&&typeof e.labels=="object"&&e.
labels.session?e.labels.session:{}},totals:{credits:Number(l.credits)||0,turns:Number(l.turns)||0}}}}var me=()=>({credits:0,
turns:0});function ge(t,e){return e==="credits"?t.credits:e==="turns"?t.turns:t.turns?t.credits/t.turns:0}function k(t){
...
(r===""||u<r)})}function Re(t,e){if(!t)return"";let r=Date.parse(`${t.slice(0,10)}T${t.slice(11,13)}:00:00Z`);return new Date(
r+e*36e5).toISOString().slice(0,13)}function Be(t,e){if(!t||!e)return 0;let r=Date.parse(`${t.slice(0,10)}T${t.slice(11,
13)}:00:00Z`),s=Date.parse(`${e.slice(0,10)}T${e.slice(11,13)}:00:00Z`);return Math.round((s-r)/36e5)}function fe(t,e){if(e===
"all")return{current:t.rows,prior:[],lo:""};if(e==="cycle"){let{start_hour:l,prev_start_hour:d}=t.cycle,c=t.dims.hours,i=c.
length?c[c.length-1]:l,g=Math.max(1,Be(l,i)+1),f=Re(d,g);return f>l&&(f=l),{current:F(t,l,""),prior:F(t,d,f),lo:l}}let r=Number(
e),s=ce(t,r*24),u=ce(t,r*48);return{current:F(t,s,""),prior:F(t,u,s),lo:s}}function he(t,e){return e==="cycle"?Math.max(
1,Math.round((Date.now()-Date.parse(t.cycle.start_utc))/864e5)):e==="all"?0:Number(e)}function X(t){let e=me();for(let r of t)
e.credits+=r[v.credits],e.turns+=r[v.turns];return e}function B(t,e){let r=new Map;for(let s of t){let u=e(s),l=r.get(u);
l||r.set(u,l=me()),l.credits+=s[v.credits],l.turns+=s[v.turns]}return r}var A=(t,e,r)=>t.dims[r][e[v[r]]],K=(t,e,r)=>{let s=t.
dims.hours[e[v.hour]];return r==="hour"?s:s.slice(0,10)},Ae="Unscheduled \xB7 ",be=t=>t.startsWith(Ae),ue={auto:"Auto \u2014 \
backend chose, no id reported","(not reported)":"No model reported"};function E(t,e,r){if(e==="session"){let s=t.labels.
session?.[r];return s||r}return e==="model"&&ue[r]?ue[r]:r||"(unlabelled)"}function ye(t,e,r){return e==="session"&&t.labels.
session?.[r]?r:""}function xe(t,e,r,s){let u=B(e,d=>A(t,d,s)),l=B(r,d=>A(t,d,s));return[...u.entries()].sort((d,c)=>c[1].
credits-d[1].credits||d[0].localeCompare(c[0])).map(([d,c])=>{let i=l.get(d);return{key:d,cell:c,deltaPct:i&&i.credits>0?
(c.credits-i.credits)/i.credits*100:null}})}function U(t,e){return e>0?(t-e)/e*100:null}import{Fragment as Se,jsx as n,jsxs as p}from"react/jsx-runtime";var ke="usage-lens-styles",je=8,Fe=5;function Ue(){Ne(()=>{
if(document.getElementById(ke))return;let t=document.createElement("style");t.id=ke,t.textContent=ie,document.head.appendChild(
...
"ul-down"}`,children:[r>0?"\u25B2":"\u25BC"," ",Math.abs(r),"% ",e]})}function Ge(){Ue();let t=Ke(),[e,r]=N(null),[s,u]=N(
!1),[l,d]=N(""),[c,i]=N(!0),[g,f]=N("cycle"),[b,h]=N("hour"),[m,z]=N("model"),[C,P]=N("credits"),$=we(()=>{try{return Intl.
DateTimeFormat().resolvedOptions().timeZone||""}catch{return""}},[]),_=()=>{i(!0),t.get(`/api/apps/usage-lens/series?day\
s=0${$?`&tz=${encodeURIComponent($)}`:""}`).then(a=>{let{series:M,stale:T}=pe(a);r(M),u(T),d("")}).catch(a=>d(a instanceof
Error?a.message:String(a))).finally(()=>i(!1))};Ne(_,[$]);let o=we(()=>{if(!e)return null;let{current:a,prior:M}=fe(e,g),
T=X(a),$e=X(M),ee=xe(e,a,M,m),j=ee.slice(0,je),Z=new Map(j.map((x,R)=>[x.key,q[R%q.length]])),te=[...new Set(a.map(x=>K(
e,x,b)))].sort(),_e=j.map(x=>{let R=B(a.filter(w=>A(e,w,m)===x.key),w=>K(e,w,b));return{name:E(e,m,x.key),color:Z.get(x.
key),values:te.map(w=>{let D=R.get(w);return D?ge(D,C):0})}}),re=[...new Set(a.map(x=>K(e,x,"day")))].sort(),Me=j.slice(
0,Fe).map(x=>{let R=B(a.filter(w=>A(e,w,m)===x.key),w=>K(e,w,"day"));return{name:E(e,m,x.key),color:Z.get(x.key),values:re.
map(w=>{let D=R.get(w);return D&&D.turns?D.credits/D.turns:null})}});return{now:T,before:$e,rows:ee,top:j,colours:Z,buckets:te,
byBucket:_e,unitLabels:re,unit:Me,distinct:new Set(a.map(x=>x[v[m]])).size,reconcile:g==="cycle"&&typeof e.official.credits_used==
"number"?{official:e.official.credits_used,local:T.credits,gap:e.official.credits_used-T.credits,coveragePct:e.official.
credits_used?T.credits/e.official.credits_used*100:null}:null}},[e,g,b,m,C]),O=Y.find(a=>a.key===m).label,Ce=V.find(a=>a.
key===C).label;return p("div",{className:"ul-root",children:[n(We,{title:"Usage Lens",subtitle:"Credits by model, surfac\
...
Window",options:de.map(a=>({key:a.key,label:a.label})),value:g,onChange:f}),n(H,{label:"Granularity",options:[{key:"hour",
label:"Hourly"},{key:"day",label:"Daily"}],value:b,onChange:h}),n(H,{label:"Break down by",options:Y,value:m,onChange:z}),
n(H,{label:"Metric",options:V,value:C,onChange:P})]}),l?n(ve,{title:"Could not read usage data",subtitle:l,action:n(J,{onClick:_,
children:"Try again"})}):!e||!o?n(Ie,{rows:6}):e.rows.length===0?n(ve,{title:"No usage recorded yet",subtitle:"The gatew\
ay writes one row per agent turn to usage/tokens. Run a turn, then refresh.",action:n(J,{onClick:_,children:"Refresh"})}):
p(Se,{children:[s&&n("div",{className:"ul-flag ul-flag-info",children:p("div",{children:[n("div",{className:"ul-flag-tit\
...
o.distinct)})]}),o.reconcile&&p(I,{style:{marginBottom:14},children:[n(W,{children:"Against Kiro's own meter \u2014 this bill\
...
ul-mono",children:[e.cycle.start_hour.replace("T"," "),":00"]}),e.cycle.resets?p(Se,{children:[" ","to ",n("span",{className:"\
ul-mono",children:e.cycle.resets})," UTC"]}):null," \xB7 ","day ",he(e,"cycle"),e.cycle.source==="kiro-api"?"":" \xB7 bound\
ary assumed"]})]}),p("div",{className:"ul-charts",children:[p(I,{children:[n(W,{children:`${Ce} over time \u2014 ${b==="\
hour"?"hourly":"daily"}, stacked by ${O.toLowerCase()}`}),n(se,{labels:o.buckets,series:o.byBucket,labelOf:a=>b==="hour"?
`${a.slice(5,10)} ${a.slice(11,13)}h`:a.slice(5),unit:C==="turns"?" turns":" credits"}),n(le,{items:o.top.map(a=>({name:E(
e,m,a.key),color:o.colours.get(a.key)}))})]}),p(I,{children:[n(W,{children:`Share of credits by ${O.toLowerCase()}`}),n(
oe,{slices:o.top.map(a=>({name:E(e,m,a.key),value:a.cell.credits,color:o.colours.get(a.key)}))})]})]}),p(I,{style:{marginBottom:14},
children:[n(W,{children:"Credits per turn, by day"}),n(ae,{labels:o.unitLabels.map(a=>a.slice(5)),series:o.unit})]}),p(I,
...
edits / turn"}),n("th",{children:"vs prior window"})]})}),n("tbody",{children:o.rows.map(a=>{let M=ye(e,m,a.key);return p(
"tr",{children:[p("td",{children:[p("span",{className:`ul-name${be(a.key)?" ul-unscheduled":""}`,children:[n("span",{className:"\
ul-sw",style:{background:o.colours.get(a.key)||"var(--border-strong, var(--border))"}}),n("span",{children:E(e,m,a.key)})]}),
...
a.deltaPct),"%"]})})]},a.key)})})]})}),n("p",{className:"ul-foot",children:`${e.shards} daily shard${e.shards===1?"":"s"}\
 \xB7 times in ${e.tz} \xB7 generated ${e.generated_at.replace("T"," ").slice(0,16)}`})]})]})]})]})}export{Ge as default};
